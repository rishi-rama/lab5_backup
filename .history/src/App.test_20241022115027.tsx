...
describe('Budget Tracker Application', () => {
  ...
  test('can add a new expense and updates totals correctly', async () => {
    ...
    // Verify expense appears in list and totals update
  ...
  test('can edit budget value', async () => {
    ...
    // Using getAllByRole since there might be multiple number inputs
    const budgetInputs = screen.getAllByRole('spinbutton');
    // Get the first spinbutton (should be the budget input)
    const budgetInput = budgetInputs[0];
    
    await userEvent.clear(budgetInput);
    await userEvent.type(budgetInput, '2000');
    
    // Find the save button in the Budget component
